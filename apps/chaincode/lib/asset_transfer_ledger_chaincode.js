...
class Chaincode extends Contract {
  ...
  async CreateAsset(ctx, assetID, area, location, owner, status) {
    ...
    let statusIdIndexKey = await ctx.stub.createCompositeKey(indexName, [
      asset.status,
      asset.assetID,
    ]);
  ...
  async DeleteAsset(ctx, id) {
    ...
    // delete the index
    let indexName = "status~id";
    let statusIdIndexKey = ctx.stub.createCompositeKey(indexName, [
      assetJSON.status,
      assetJSON.assetID,
    ]);
    if (!statusIdIndexKey) {
  ...
  async HoldAsset(ctx, assetID) {
    ...
    assetToHold.status = "hold"; //Hold (lock) the asset

    let assetJSONasBytes = Buffer.from(JSON.stringify(assetToHold));
    await ctx.stub.putState(assetID, assetJSONasBytes); //rewrite the asset
  }
  ...
  async UnHoldAsset(ctx, assetID) {
    ...
    assetToUnhold.status = "registered"; //Unhold (unlock) the asset
